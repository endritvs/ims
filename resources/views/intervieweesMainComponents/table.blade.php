      ...
      <tbody>
        ...
        <th scope="row" class="py-4 px-6 font-medium whitespace-nowrap text-white">
          {{$i->interviewee_type->name}} <!-- Qet Emrin e Userit -->
              ...
              <th scope="row" class="py-4 px-6 font-medium whitespace-nowrap text-white">
                @foreach ($exec as $a)
            @if ($i->interviewee_type->name===$a->name)
              {{$a->Attributes}}
            @endif
...
<div id="addUserModal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 w-full md:inset-0 h-modal md:h-full">
  <div class="relative p-4 w-full max-w-md h-full md:h-auto">
      ...
      <form method="POST" action="{{ route('interviewees.store') }}" enctype="multipart/form-data" class="relative bg-white rounded-lg shadow dark:bg-gray-700">
        ...
        <select name="interviewee_attributes_id[]"  class="@error('interviewee_attributes_id') is-invalid @enderror bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" id="interviewee_attributes_id" multiple="multiple">
          @foreach ($intervieweesAt as $a)   
