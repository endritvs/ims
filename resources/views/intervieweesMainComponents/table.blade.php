      ...
      <tbody>
        ...
        @endphp
          ...
          <tr class="bg-white border-b bg-gray-800 border-gray-700">
              ...
              <th scope="row" class="py-4 px-6 font-medium whitespace-nowrap text-white">
                {{ $i -> surname }} <!-- Qet Emrin e Userit -->
            </th>
            <th scope="row" class="py-4 px-6 font-medium whitespace-nowrap text-white underline">
              <a href="/storage/cv_path/{{$cv[2]}}" download>Download CV</a> <!-- Qet Emrin e Userit -->
          </th>

          <th scope="row" class="py-4 px-6 font-medium whitespace-nowrap text-white underline">
            <a href="{{ $i -> external_cv_path }}">External Link</a> <!-- Qet Emrin e Userit -->
        </th>
        {{-- {{$i->interviewee_type->name}} --}}
        <th scope="row" class="py-4 px-6 font-medium whitespace-nowrap text-white">
          {{$i->interviewee_type->name}} <!-- Qet Emrin e Userit -->
      </th>
       
              <th scope="row" class="py-4 px-6 font-medium whitespace-nowrap text-white">
                {{ $i->interviewee_attribute->name }} <!-- Qet Emrin e Userit -->
            </th>
           
            <th scope="row" class="py-4 px-6 font-medium whitespace-nowrap text-white">
              <img src="/storage/images/{{ $link[2] }}" width="50px" height="50px"> <!-- Qet Emrin e Userit -->
          </th>

              <td class="py-4 px-6 text-blue-600">
                  <a href="{{route('interviewees.edit',$i->id)}}">Edit</a> <!-- Edit Button Here -->
              </td>
              <td class="py-4 px-6 text-red-600">
                  {{-- <a href="{{ route('interviewees.destroy', $i -> id) }}">Delete</a>   --}}
...
<div id="addUserModal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 w-full md:inset-0 h-modal md:h-full">
  <div class="relative p-4 w-full max-w-md h-full md:h-auto">
      ...
      <form method="POST" action="{{ route('interviewees.store') }}" enctype="multipart/form-data" class="relative bg-white rounded-lg shadow dark:bg-gray-700">
          ...
          <div class="p-6 space-y-6">
              <div class="space-y-6">
                  <div>
                    ...
                    <input type="text" name="name" id="name" autocomplete="given-name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" placeholder="Name" required>
                </div>
                <div>
                  <label for="name" class="block  text-sm font-medium text-gray-900 dark:text-gray-300">Surname</label>
                  <input type="text" name="surname" id="surname" autocomplete="given-name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" placeholder="Surname" required>
              </div>
              <div>
                <label for="name" class="block text-sm font-medium text-gray-900 dark:text-gray-300">Insert CV</label>
                <input type="file" name="cv_path" id="cv_path" autocomplete="given-name" class="bg-gray-50 " placeholder="cv_path" required>
            </div>
            <div>
              <label for="name" class="block text-sm font-medium text-gray-900 dark:text-gray-300">External CV Path</label>
              <input type="text" name="external_cv_path" id="name" autocomplete="given-name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" placeholder="External CV Path" required>
          </div>
          <div>
            <label for="name" class="block  text-sm font-medium text-gray-900 dark:text-gray-300">Interviewee Types</label>
        ...
        <div>
          <label for="name" class="block  text-sm font-medium text-gray-900 dark:text-gray-300">Interviewee Attributes</label>
          {{-- <input type="text" name="interviewee_attributes_id" id="interviewee_attributes_id" autocomplete="given-name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" placeholder="Interviewee Attributes" required> --}}
          <select class="@error('interviewee_attributes_id') is-invalid @enderror" name="interviewee_attributes_id" id="interviewee_attributes_id">
            @foreach ($intervieweesAt as $i)   
            <option value="{{$i->id}}">{{$i->name}}</option>
           @endforeach
        </select>
        </div>
      <div>
        <label for="name" class="block  text-sm font-medium text-gray-900 dark:text-gray-300">Insert Image</label>
        <input type="file" name="img" id="img" autocomplete="given-name" class="bg-gray-50 " placeholder="cv_path" required>
      </div>
