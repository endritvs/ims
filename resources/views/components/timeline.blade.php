            ...
            <ol class="text-center sm:flex no-scrollbar overflow-x-auto h-auto">
                ...
                @for ($index = 0; $index < count($interview); $index++)
                    ...
                    <div class="flex items-center mt-100 relative ">
                        ...
                        @else
                            <div
                                class="absolute w-[195px] px-10 flex z-10 justify-center items-center h-10 bg-blue-200 rounded-full ring-0 ring-white dark:bg-blue-900 sm:ring-8 dark:ring-gray-900 shrink-0">
                    ...
                    <div class="relative block p-6 overflow-hidden border border-gray-400 rounded-xl">
                        ...
                        <div class="justify-between items-center sm:flex">
                            <div>
                                <h2 class="text-[30px] pl-[15px]  font-bold text-black">
                        ...
                        <div class="space-y-4">
                            ...
                            <p class="text-[25px] font-[500] text-black">
                            ...
                            <p
                                class="text-[14px] inline-flex items-center font-bold leading-sm uppercase px-3 bg-blue-200 text-blue-700 rounded-full">
                                |
                            ...
                            <div class="flex justify-center">
                                @if ($today > $date &&
                                    !App\Models\review::where('candidate_id', $interview[$index]->interviewees->id)->where('questionnaire_id', $interview[$index]->user->id)->where('interview_id', $interview[$index]->id)->exists())
                                    <button data-modal-toggle="RatingModal{{ $interview[$index]->id }}"
                                        class="bg-blue-700 text-white p-1 rounded">Rate Now</button>
                                @else
                                    <button data-tooltip-target="tooltip-default"
                                        class="bg-red-500  text-white p-1 rounded">Can't rate now!</button>
                        ...
                        <div id="RatingModal{{ $interview[$index]->id }}" tabindex="-1"
                            class="hidden  overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 w-full md:inset-0 h-modal md:h-full justify-center items-center flex"
                            aria-modal="true" role="dialog">
                            <div class="relative p-4 w-auto max-w-2xl h-full md:h-auto">
                                ...
                                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                    ...
                                    <div
                                        ...
                                        <button type="button"
                                            class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white"
                                            data-modal-toggle="RatingModal{{ $interview[$index]->id }}">
                                            <svg aria-hidden="true" class="w-5 h-5" fill="currentColor"
                                                viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                                                <path fill-rule="evenodd"
                                                    d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z"
                                        ...
                                        <form method="POST" action="{{ route('review.store') }}"
                                            ...
                                            <div class="hidden">
                                                ...
                                                <select name="candidate_id" id="candidate_id"
                                                    ...
                                                    <option class="capitalize"
                                                        value="{{ $interview[$index]->interviewees->id }}">
                                                        {{ $interview[$index]->interviewees->name }}</option>
                                                </select>
                                                @error('candidate_id')
                                            ...
                                            <div>
                                                ...
                                                <select name="questionnaire_id" id="questionnaire_id"
                                                    class="hidden @error('questionnaire_id') is-invalid @enderror capitalize bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white ">
                                                    <option class="capitalize hidden"
                                                        value="{{ $interview[$index]->user->id }}">
                                                        {{ $interview[$index]->user->name }}</option>
                                                </select>
                                                @error('questionnaire_id')
                                                    <div class="ml-1 text-red-500 text-xs alert alert-danger hidden">
                                                        {{ $message }}</div>
                                                @enderror
                                            </div>
                                            <div>

                                                <input value="{{ $interview[$index]->id }}" name="interview_id"
                                                    id="interview_id"
                                                    class="hidden @error('interview_id') is-invalid @enderror capitalize bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white "
                                                    type="hidden">
                                            ...
                                            <div>
                                                ...
                                                <div class="rating rating-lg">
                                                    <input type="radio" name="rating_amount" id="star0"
                                                        class="rating-hidden" />
                                                    <input type="radio" value="1" name="rating_amount"
                                                        id="star1"
                                                        class="bg-green-500 mask mask-star-2 @error('rating_amount') is-invalid @enderror" />
                                                    <input type="radio" value="2" name="rating_amount"
                                                        id="star3"
                                                        class="bg-green-500 mask mask-star-2 @error('rating_amount') is-invalid @enderror" />
                                                    <input type="radio" value="3" name="rating_amount"
                                                        id="star5"
                                                        class="bg-green-500 mask mask-star-2 @error('rating_amount') is-invalid @enderror" />
                                                    <input type="radio" value="4" name="rating_amount"
                                                        id="star7"
                                                        class="bg-green-500 mask mask-star-2 @error('rating_amount') is-invalid @enderror" />
                                                    <input type="radio" value="5" name="rating_amount"
                                                        id="star9"
                                                        class="bg-green-500 mask mask-star-2 @error('rating_amount') is-invalid @enderror" />
                                                </div>
            ...
            <div class="w-full overflow-hidden rounded-lg shadow-xs">
                <div class="w-full overflow-x-auto">
                    <table class="w-full">
                        ...
                        <thead>
                            <tr
                                class="text-xs font-semibold tracking-wide text-left text-black uppercase border-b dark:border-gray-700 bg-gray-50 dark:text-white dark:bg-gray-800">
                        ...
                        <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                            @foreach ($pastInterview as $i)
                                ...
                                @endphp
                                <tr
                                    class="bg-gray-50 dark:bg-gray-800 hover:bg-gray-100 dark:hover:bg-gray-900 text-black dark:text-white">
