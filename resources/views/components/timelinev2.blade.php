...
<div class="w-full max-w-full mt-6 shrink-0 md:w-4/12 md:flex-0 md:mt-0 xl:w-1/4">
    <div class="relative flex flex-col min-w-0 break-words bg-white border-0 shadow-xl dark:bg-slate-850 dark:shadow-dark-xl rounded-2xl bg-clip-border">
        ...
        <div class="flex flex-col items-center -mx-3">
            ...
            <p class=" dark:text-white/80">{{ $interview[$index]->interviewees->interviewee_type->name }}</p>
        </div>
        ...
        <div class="border-black/12.5 rounded-t-2xl p-8 text-center pt-0 pb-6 lg:pt-2 lg:pb-4">
            <div class="flex gap_5">
                ...
                <label class="w-4/12 md:w-1/2 px-2 py-1 mx-1 font-bold leading-normal text-center text-white align-middle transition-all ease-in border-0 rounded-lg shadow-md cursor-pointer text-xs bg-cyan-500 lg:block tracking-tight-rem hover:shadow-xs hover:-translate-y-px active:opacity-85">
                    {{ $s->name }}
                </label>
                @endforeach
            </div>
        ...
        <div class="flex-auto pt-0">
            <div class="text-center">
                <div class="my-2 dark:text-white/80">
                   <p class="text-md dark:text-white">Time of interview: {{ $interview[$index]->interview_date }}</p>
                </div>
            </div>
        </div>
...
<div id="defaultModal{{ $interview[$index]->id }}" tabindex="-1"
    ...
    <div class="relative p-4 w-full max-w-2xl h-full md:h-auto">
        ...
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <p class="text-center dark:text-white text-sm">Questioners:</p> 
            <div class="flex flex-row pb-4 px-6">
            
            @foreach($intervieweeName as $key => $iName)
            <div class="items-center">
                <img class="inline-flex w-12 h-12 mx-auto text-white transition-all duration-200 ease-in-out rounded-circle" src="/storage/img/{{$icon[$key]}}">
...
<div class="w-full max-w-full mt-6 shrink-0 md:w-4/12 md:flex-0 md:mt-0 xl:w-1/4">
    <div class="relative flex flex-col min-w-0 break-words bg-white border-0 shadow-xl dark:bg-slate-850 dark:shadow-dark-xl rounded-2xl bg-clip-border">
        ...
        <div class="flex flex-col items-center -mx-3">
            ...
            <p class="my-2 dark:text-white/80">{{ $iAll['0']['interviewees']['interviewee_type']['name'] }}</p>
        </div>
