...
<form class="shadow rounded-md w-[500px]" method="POST" action="{{ route('interview.update',$interview->id) }}">
    ...
    <div class="p-4">
        <input type="hidden" value="{{$interview->interview_id}}" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" name="interview_name" value={{ $interview -> interview_name }}>
        </div>
 ...
 @enderror
    ...
    <div class="p-4">
    <input type="date" min=<?php echo date('Y-m-d'); ?> class="@error('interview_date') is-invalid @enderror mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" name="interview_date" value={{ $interview -> interview_date }}>
